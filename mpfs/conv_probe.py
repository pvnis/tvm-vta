...
def run_conv2d(env, remote, wl, target, check_correctness=True, print_ir=False,
    ...
    temp = utils.tempdir()
    # The stock test saves a .o and lets the RPC server link it. This board's runtime cannot,
    # so export a shared library with the riscv64 cross compiler, the way every other probe
    # in this tree does.
    if env.TARGET == "mpfs":
        path = temp.relpath("conv2d.so")
        mod.export_library(path, cc=os.environ.get(
            "VTA_CROSS_CC", "/home/dmd/polarfire_sandbox/vta/tools/rvcc/rv64gc-gcc"))
        remote.upload(path)
...
def test_conv2d(device=os.environ.get("CONV_DEV", "vta")):
    def _run(env, remote):
        if device == "vta":
            target = env.target
            # mpfs: the fabric is programmed over JTAG by reprogram_board.sh, not pushed
            # over RPC, and there is no prebuilt bitstream to download for this config.
            if env.TARGET not in ["sim", "tsim", "intelfocl", "mpfs"]:
        ...
        with autotvm.tophub.context(target):  # load pre-tuned schedule parameters
            ...
            wkls = [(n, w) for (n, w) in resnet_wkls if not only or n.endswith("." + only)]
            print(f"[conv] running {len(wkls)} workload(s): {[n for n, _ in wkls]}", flush=True)
            for _, wl in wkls:
                print(wl)
